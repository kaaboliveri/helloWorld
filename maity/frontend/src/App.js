...
let messageIdCounter = 2; // Initial messages count + 1
...
function App() {
  ...
  useEffect(() => {
    ...
    if (conversationId && currentView === 'chat') {
      ...
      webSocket.current.onopen = () => { console.log('WebSocket connected'); setIsConnected(true); setError(null); };
      webSocket.current.onmessage = (event) => {
        setIsAiTyping(false);
        try {
            const receivedData = JSON.parse(event.data);
            console.log('WS Receive:', receivedData);
        ...
        } catch (e) { console.error('Error processing WebSocket message:', e); setError('Malformed data from server.');}
      };
      webSocket.current.onerror = (err) => { console.error('WebSocket error:', err); setError('WebSocket connection error.'); setIsConnected(false); setIsAiTyping(false);};
      webSocket.current.onclose = (event) => { console.log('WebSocket disconnected:', event.reason, `Code: ${event.code}`); setIsConnected(false); setIsAiTyping(false);};
  ...
  const handleSendMessage = async (text) => {
    if (currentView !== 'chat' || !webSocket.current || webSocket.current.readyState !== WebSocket.OPEN) { setError('Not connected to Maity AI for chat.'); return; }
    const newUserMessage = { id: messageIdCounter++, text: text, sender: 'user' };
    setMessages(prev => [...prev, newUserMessage]);
    setIsAiTyping(true); setError(null);
    webSocket.current.send(JSON.stringify({ message: text, config: { preferred_model: selectedModel } }));
  };
  useEffect(() => { const el = document.querySelector('.message-list'); if (el) el.scrollTop = el.scrollHeight; }, [messages]);

  const handleNewChat = () => {
    if (webSocket.current && webSocket.current.readyState === WebSocket.OPEN) webSocket.current.close();
    setCurrentView('chat');
  ...
  const handleSetupMonitorSubmit = async (event) => {
    ...
    try {
      ...
      fetchActiveMonitorTasks(); // Refresh task list after successful setup
    } catch (err) {
  ...
  }, [currentView]); // monitorStatusMessage removed, fetch on successful setup instead


  const renderChatView = () => (
    <div className="chat-view-container">
      <div className="chat-window">
        {!isConnected && !error && currentView === 'chat' && <div className="connection-indicator">Connecting...</div>}
        {error && <div className="error-indicator">{error}</div>}
        <MessageList messages={messages} />
        {isAiTyping && <div className="typing-indicator">Maity is thinking...</div>}
        <MessageInput onSendMessage={handleSendMessage} isAiTyping={isAiTyping || (currentView === 'chat' && !isConnected)} />
      </div>
    </div>
  );
    ...
    <div className="monitoring-view-container">
      <div className="monitor-section">
        ...
        <form onSubmit={handleSetupMonitorSubmit} className="monitor-setup-form">
          ...
          <button type="submit" className="submit-monitor-button" disabled={isSubmittingMonitor}>
            {isSubmittingMonitor ? 'Setting up...' : 'Setup Monitoring Task'}
          </button>
          {monitorStatusMessage && (
            <div className={`monitor-status-message ${monitorStatusMessage.includes('Error:') ? 'error' : 'success'}`}>
              {monitorStatusMessage}
            </div>
          )}
        </form>
      ...
      <div className="monitor-section">
        ...
        <button onClick={fetchActiveMonitorTasks} disabled={isLoadingTasks} className="refresh-tasks-button">
          {isLoadingTasks ? 'Refreshing...' : 'Refresh Active Tasks'}
        </button>
            ...
            {activeMonitorTasks.map(task => (
              <li key={task.id} className="monitor-task-item">
                <div className="task-info">
                  ...
                  <strong>Frequency:</strong> {task.frequency_hours}h <br />
                  <strong>Next Run:</strong> {task.next_run ? new Date(task.next_run).toLocaleString() : 'N/A'}
                </div>
                <button onClick={() => fetchTaskResults(task.id)} disabled={isLoadingResults && selectedTaskResults?.taskId === task.id} className="view-results-button">
                  {isLoadingResults && selectedTaskResults?.taskId === task.id ? 'Loading...' : 'View Results'}
                </button>
                {selectedTaskResults && selectedTaskResults.taskId === task.id && (
                  <div className="task-results-display">
                    <h4>Results for: {task.topic}</h4>
                    {selectedTaskResults.isError && <p className="error-text">{selectedTaskResults.message}</p>}
                    {!selectedTaskResults.isError && selectedTaskResults.results.length === 0 && <p>{selectedTaskResults.message || "No results available."}</p>}
                    {selectedTaskResults.results.length > 0 && (
                      <ul>
                        {selectedTaskResults.results.map((finding, index) => (
                          <li key={index} className="finding-item">
                            {finding.timestamp && <span className="finding-timestamp">{new Date(finding.timestamp).toLocaleString()}: </span>}
                            <div className="finding-title">
                                <ReactMarkdown remarkPlugins={[remarkGfm]}>{finding.title || 'N/A'}</ReactMarkdown>
                            </div>
                            {finding.source_url && <a href={finding.source_url} target="_blank" rel="noopener noreferrer" className="finding-source"> (Source)</a>}
                          </li>
                        ))}
                      </ul>
                    )}
    ...
    <div className="app-container">
      <header className="app-header">
        ...
        <div className="header-controls">
          ...
          {currentView === 'monitoring' && ( <div className="header-placeholder" style={{minWidth: '220px'}}></div> /* Ensure placeholder takes similar space */)}
        </div>
      </header>
      {currentView === 'chat' ? renderChatView() : renderMonitoringView()}
    </div>
  );
}
export default App;
